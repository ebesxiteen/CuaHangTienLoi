package GUI;

import java.awt.BorderLayout;
import java.awt.Color;
import java.awt.Dimension;
import java.awt.GridBagConstraints;
import java.awt.GridBagLayout;
import java.awt.GridLayout;
import java.awt.Insets;
import java.awt.event.MouseAdapter;
import java.awt.event.MouseEvent;
import java.time.LocalDate;
import java.time.format.DateTimeFormatter;
import java.time.format.DateTimeParseException;
import java.util.ArrayList;

import javax.swing.BorderFactory;
import javax.swing.JButton;
import javax.swing.JComboBox;
import javax.swing.JLabel;
import javax.swing.JOptionPane;
import javax.swing.JPanel;
import javax.swing.JScrollPane;
import javax.swing.JSplitPane;
import javax.swing.JTable;
import javax.swing.JTextField;
import javax.swing.ListSelectionModel;
import javax.swing.SwingUtilities;
import javax.swing.event.DocumentEvent;
import javax.swing.event.DocumentListener;
import javax.swing.table.DefaultTableCellRenderer;
import javax.swing.table.DefaultTableModel;

import BLL.NhanVienBLL;
import DTO.nhanVien;

public class NhanVienView extends javax.swing.JPanel {

    DefaultTableModel model;
    private nhanVien nv = new nhanVien();
    private ArrayList<nhanVien> list = new ArrayList<>();
    NhanVienBLL nvBll = new NhanVienBLL(this);
    private int count = 0;
    private long luong;
    private String matmp, hotmp, tentmp, gioitinhtmp, sdttmp, ngaysinhtmp, chucvutmp, luongTexttmp;
    private boolean updatingFilters;

    public javax.swing.JButton btn_luu;
    public javax.swing.JButton btn_reset;
    public javax.swing.JButton btn_sua;
    public javax.swing.JButton btn_them;
    public javax.swing.JButton btn_xoa;
    public javax.swing.JComboBox<String> chucvu_nv;
    public javax.swing.JComboBox<String> gioitinh_nv;
    public javax.swing.JTextField ho_nv;
    public javax.swing.JTextField luong_nv;
    public javax.swing.JTextField ma_nv;
    public javax.swing.JTextField ngaysinh_nv;
    public javax.swing.JTextField sdt_nv;
    public javax.swing.JTable table_nv;
    public javax.swing.JTextField ten_nv;

    private JButton btnSearch;
    private JTextField txtSearch;
    private JComboBox<String> cboMaNVFilter;
    private JComboBox<String> cboGioiTinhFilter;
    private JComboBox<String> cboChucVuFilter;

    public NhanVienView() {
        initComponents();
        UnEditable();
        wireEvents();
    }

    private void initComponents() {
        setLayout(new BorderLayout(10, 10));
        setBackground(Color.WHITE);
        setPreferredSize(new Dimension(1015, 690));

        JLabel title = new JLabel("QUẢN LÝ NHÂN VIÊN");
        title.setFont(new java.awt.Font("Segoe UI", java.awt.Font.BOLD, 26));
        title.setForeground(new Color(0, 102, 102));

        txtSearch = new JTextField();
        btnSearch = new JButton("Tìm kiếm");
        btnSearch.setBackground(new Color(0, 102, 102));
        btnSearch.setForeground(Color.WHITE);
        btnSearch.setBorderPainted(false);
        txtSearch.setPreferredSize(new Dimension(240, 28));
        btnSearch.setPreferredSize(new Dimension(100, 28));

        JPanel searchPanel = new JPanel(new BorderLayout(6, 0));
        searchPanel.setBackground(Color.WHITE);
        searchPanel.add(txtSearch, BorderLayout.CENTER);
        searchPanel.add(btnSearch, BorderLayout.EAST);

        JPanel titlePanel = new JPanel(new BorderLayout(8, 0));
        titlePanel.setBackground(Color.WHITE);
        titlePanel.setBorder(BorderFactory.createEmptyBorder(10, 10, 0, 10));
        titlePanel.add(title, BorderLayout.WEST);
        titlePanel.add(searchPanel, BorderLayout.EAST);
        add(titlePanel, BorderLayout.NORTH);

        model = new DefaultTableModel(new Object[]{
            "Mã nhân viên", "Họ nhân viên", "Tên nhân viên", "Giới tính", "Ngày sinh", "Số điện thoại", "Chức vụ", "Lương"
        }, 0) {
            @Override
            public boolean isCellEditable(int row, int column) {
                return false;
            }
        };
        table_nv = new JTable(model);
        table_nv.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        table_nv.setRowSelectionAllowed(true);
        table_nv.setCellSelectionEnabled(false);
        table_nv.setRowHeight(28);
        table_nv.getTableHeader().setResizingAllowed(false);
        table_nv.getTableHeader().setReorderingAllowed(false);
        table_nv.setDefaultEditor(Object.class, null);

        DefaultTableCellRenderer centerRenderer = new DefaultTableCellRenderer();
        centerRenderer.setHorizontalAlignment(JLabel.CENTER);
        for (int i = 0; i < table_nv.getColumnModel().getColumnCount(); i++) {
            table_nv.getColumnModel().getColumn(i).setHeaderRenderer(centerRenderer);
            table_nv.getColumnModel().getColumn(i).setCellRenderer(centerRenderer);
        }

        cboMaNVFilter = new JComboBox<>();
        cboGioiTinhFilter = new JComboBox<>();
        cboChucVuFilter = new JComboBox<>();
        JPanel filterPanel = new JPanel(new GridLayout(1, 8, 0, 0));
        filterPanel.setBackground(Color.WHITE);
        filterPanel.add(cboMaNVFilter);
        filterPanel.add(new JLabel(""));
        filterPanel.add(new JLabel(""));
        filterPanel.add(cboGioiTinhFilter);
        filterPanel.add(new JLabel(""));
        filterPanel.add(new JLabel(""));
        filterPanel.add(cboChucVuFilter);
        filterPanel.add(new JLabel(""));

        JPanel tablePanel = new JPanel(new BorderLayout(8, 8));
        tablePanel.setBackground(Color.WHITE);
        tablePanel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 5));
        tablePanel.add(filterPanel, BorderLayout.NORTH);
        tablePanel.add(new JScrollPane(table_nv), BorderLayout.CENTER);
        tablePanel.setMinimumSize(new Dimension(620, 0));

        ma_nv = new JTextField();
        ho_nv = new JTextField();
        ten_nv = new JTextField();
        gioitinh_nv = new JComboBox<>(new String[]{"Nam", "Nữ", "Khác"});
        ngaysinh_nv = new JTextField();
        sdt_nv = new JTextField();
        chucvu_nv = new JComboBox<>(new String[]{"Quản lý", "Nhân viên thu ngân", "Nhân viên tạp vụ"});
        luong_nv = new JTextField();

        btn_them = new JButton("Thêm");
        btn_sua = new JButton("Sửa");
        btn_reset = new JButton("Reset");
        btn_xoa = new JButton("Xóa");
        btn_luu = new JButton("Lưu");
        btn_them.setBackground(new Color(102, 102, 102));
        btn_sua.setBackground(new Color(102, 102, 102));
        btn_reset.setBackground(new Color(102, 102, 102));
        btn_xoa.setBackground(new Color(153, 0, 0));
        btn_luu.setBackground(new Color(0, 102, 0));
        btn_them.setForeground(Color.WHITE);
        btn_sua.setForeground(Color.WHITE);
        btn_reset.setForeground(Color.WHITE);
        btn_xoa.setForeground(Color.WHITE);
        btn_luu.setForeground(Color.WHITE);

        JPanel actionTop = new JPanel(new GridLayout(1, 3, 8, 0));
        actionTop.setBackground(Color.WHITE);
        actionTop.setPreferredSize(new Dimension(0, 38));
        actionTop.add(btn_them);
        actionTop.add(btn_sua);
        actionTop.add(btn_reset);

        JPanel infoPanel = new JPanel(new GridBagLayout());
        infoPanel.setBackground(Color.WHITE);
        infoPanel.setBorder(BorderFactory.createTitledBorder("Thông tin nhân viên"));
        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(5, 6, 5, 6);
        gbc.fill = GridBagConstraints.HORIZONTAL;
        addFormRow(infoPanel, gbc, 0, "Mã nhân viên:", ma_nv);
        addFormRow(infoPanel, gbc, 1, "Họ nhân viên:", ho_nv);
        addFormRow(infoPanel, gbc, 2, "Tên nhân viên:", ten_nv);
        addFormRow(infoPanel, gbc, 3, "Giới tính:", gioitinh_nv);
        addFormRow(infoPanel, gbc, 4, "Ngày sinh:", ngaysinh_nv);
        addFormRow(infoPanel, gbc, 5, "Số điện thoại:", sdt_nv);
        addFormRow(infoPanel, gbc, 6, "Chức vụ:", chucvu_nv);
        addFormRow(infoPanel, gbc, 7, "Lương:", luong_nv);

        JPanel actionBottom = new JPanel(new GridLayout(1, 2, 8, 0));
        actionBottom.setBackground(Color.WHITE);
        actionBottom.setPreferredSize(new Dimension(0, 38));
        actionBottom.add(btn_xoa);
        actionBottom.add(btn_luu);

        JPanel formPanel = new JPanel(new BorderLayout(8, 8));
        formPanel.setBackground(Color.WHITE);
        formPanel.setBorder(BorderFactory.createEmptyBorder(10, 5, 10, 10));
        formPanel.setMinimumSize(new Dimension(380, 0));
        formPanel.add(actionTop, BorderLayout.NORTH);
        formPanel.add(infoPanel, BorderLayout.CENTER);
        formPanel.add(actionBottom, BorderLayout.SOUTH);

        JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, tablePanel, formPanel);
        splitPane.setResizeWeight(0.62);
        splitPane.setDividerLocation(620);
        splitPane.setBorder(null);
        add(splitPane, BorderLayout.CENTER);
    }

    private void addFormRow(JPanel panel, GridBagConstraints gbc, int row, String label, java.awt.Component input) {
        gbc.gridy = row;
        gbc.gridx = 0;
        gbc.weightx = 0;
        panel.add(new JLabel(label), gbc);
        gbc.gridx = 1;
        gbc.weightx = 1;
        panel.add(input, gbc);
    }

    private void wireEvents() {
        table_nv.addMouseListener(new MouseAdapter() {
            @Override
            public void mousePressed(MouseEvent e) {
                if (!SwingUtilities.isLeftMouseButton(e)) {
                    return;
                }
                int row = table_nv.getSelectedRow();
                if (row >= 0) {
                    ma_nv.setText(String.valueOf(table_nv.getValueAt(row, 0)));
                    ho_nv.setText(String.valueOf(table_nv.getValueAt(row, 1)));
                    ten_nv.setText(String.valueOf(table_nv.getValueAt(row, 2)));
                    gioitinh_nv.setSelectedItem(String.valueOf(table_nv.getValueAt(row, 3)));
                    ngaysinh_nv.setText(String.valueOf(table_nv.getValueAt(row, 4)));
                    sdt_nv.setText(String.valueOf(table_nv.getValueAt(row, 5)));
                    chucvu_nv.setSelectedItem(String.valueOf(table_nv.getValueAt(row, 6)));
                    luong_nv.setText(String.valueOf(table_nv.getValueAt(row, 7)));
                }
            }
        });

        DocumentListener searchListener = new DocumentListener() {
            @Override
            public void insertUpdate(DocumentEvent e) {
                applyFilters();
            }

            @Override
            public void removeUpdate(DocumentEvent e) {
                applyFilters();
            }

            @Override
            public void changedUpdate(DocumentEvent e) {
                applyFilters();
            }
        };
        txtSearch.getDocument().addDocumentListener(searchListener);
        btnSearch.addActionListener(e -> applyFilters());
        cboMaNVFilter.addActionListener(e -> applyFilters());
        cboGioiTinhFilter.addActionListener(e -> applyFilters());
        cboChucVuFilter.addActionListener(e -> applyFilters());

        btn_them.addActionListener(e -> btn_themActionPerformed());
        btn_luu.addActionListener(e -> btn_luuActionPerformed());
        btn_sua.addActionListener(e -> btn_suaActionPerformed());
        btn_xoa.addActionListener(e -> btn_xoaActionPerformed());
        btn_reset.addActionListener(e -> btn_resetActionPerformed());
    }

    public void loadTable(ArrayList<nhanVien> list) {
        if (list.isEmpty()) {
            return;
        }
        nhanVien nv2 = list.get(list.size() - 1);
        model.addRow(new Object[]{nv2.getManv(), nv2.getHo(), nv2.getTen(), nv2.getGioitinh(), nv2.getNgaySinh(), nv2.getSdt(), nv2.getChucVu(), nv2.getLuong()});
    }

    public void clearTable() {
        model.setRowCount(0);
    }

    public void getTable() {
        list = nvBll.getALL();
        updateFilterCombos();
        applyFilters();
    }

    private void renderTable(ArrayList<nhanVien> data) {
        model.setRowCount(0);
        for (nhanVien row : data) {
            model.addRow(new Object[]{row.getManv(), row.getHo(), row.getTen(), row.getGioitinh(), row.getNgaySinh(), row.getSdt(), row.getChucVu(), row.getLuong()});
        }
    }

    private void updateFilterCombos() {
        updatingFilters = true;
        Object selectedMa = cboMaNVFilter.getSelectedItem();
        Object selectedGioiTinh = cboGioiTinhFilter.getSelectedItem();
        Object selectedChucVu = cboChucVuFilter.getSelectedItem();

        cboMaNVFilter.removeAllItems();
        cboGioiTinhFilter.removeAllItems();
        cboChucVuFilter.removeAllItems();
        cboMaNVFilter.addItem("Tất cả mã NV");
        cboGioiTinhFilter.addItem("Tất cả giới tính");
        cboChucVuFilter.addItem("Tất cả chức vụ");

        for (nhanVien item : list) {
            addUniqueComboItem(cboMaNVFilter, item.getManv());
            addUniqueComboItem(cboGioiTinhFilter, item.getGioitinh());
            addUniqueComboItem(cboChucVuFilter, item.getChucVu());
        }

        restoreComboSelection(cboMaNVFilter, selectedMa, 0);
        restoreComboSelection(cboGioiTinhFilter, selectedGioiTinh, 0);
        restoreComboSelection(cboChucVuFilter, selectedChucVu, 0);
        updatingFilters = false;
    }

    private void addUniqueComboItem(JComboBox<String> combo, String value) {
        if (value == null || value.isBlank()) {
            return;
        }
        for (int i = 0; i < combo.getItemCount(); i++) {
            if (value.equals(combo.getItemAt(i))) {
                return;
            }
        }
        combo.addItem(value);
    }

    private void restoreComboSelection(JComboBox<String> combo, Object selectedValue, int defaultIndex) {
        if (selectedValue != null) {
            for (int i = 0; i < combo.getItemCount(); i++) {
                if (selectedValue.equals(combo.getItemAt(i))) {
                    combo.setSelectedIndex(i);
                    return;
                }
            }
        }
        combo.setSelectedIndex(defaultIndex);
    }

    private void applyFilters() {
        if (updatingFilters) {
            return;
        }
        String selectedMa = String.valueOf(cboMaNVFilter.getSelectedItem());
        String selectedGioiTinh = String.valueOf(cboGioiTinhFilter.getSelectedItem());
        String selectedChucVu = String.valueOf(cboChucVuFilter.getSelectedItem());
        String keyword = txtSearch.getText().trim().toLowerCase();

        boolean filterMa = selectedMa != null && !"Tất cả mã NV".equals(selectedMa);
        boolean filterGioiTinh = selectedGioiTinh != null && !"Tất cả giới tính".equals(selectedGioiTinh);
        boolean filterChucVu = selectedChucVu != null && !"Tất cả chức vụ".equals(selectedChucVu);
        boolean filterKeyword = !keyword.isEmpty();

        ArrayList<nhanVien> result = new ArrayList<>();
        for (nhanVien item : list) {
            boolean matchesMa = !filterMa || selectedMa.equals(item.getManv());
            boolean matchesGioiTinh = !filterGioiTinh || selectedGioiTinh.equals(item.getGioitinh());
            boolean matchesChucVu = !filterChucVu || selectedChucVu.equals(item.getChucVu());
            boolean matchesKeyword = !filterKeyword
                    || containsKeyword(item.getManv(), keyword)
                    || containsKeyword(item.getHo(), keyword)
                    || containsKeyword(item.getTen(), keyword)
                    || containsKeyword(item.getGioitinh(), keyword)
                    || containsKeyword(item.getNgaySinh(), keyword)
                    || containsKeyword(item.getSdt(), keyword)
                    || containsKeyword(item.getChucVu(), keyword)
                    || containsKeyword(String.valueOf(item.getLuong()), keyword);
            if (matchesMa && matchesGioiTinh && matchesChucVu && matchesKeyword) {
                result.add(item);
            }
        }
        renderTable(result);
    }

    private boolean containsKeyword(String value, String keyword) {
        return value != null && value.toLowerCase().contains(keyword);
    }

    public void ResetFieldText() {
        ma_nv.setText("");
        ho_nv.setText("");
        ten_nv.setText("");
        gioitinh_nv.setSelectedIndex(0);
        ngaysinh_nv.setText("");
        sdt_nv.setText("");
        luong_nv.setText("");
        chucvu_nv.setSelectedIndex(0);
    }

    public void Editable() {
        btn_luu.setEnabled(true);
        ma_nv.setEditable(false);
        ho_nv.setEditable(true);
        ten_nv.setEditable(true);
        gioitinh_nv.setEnabled(true);
        ngaysinh_nv.setEditable(true);
        sdt_nv.setEditable(true);
        luong_nv.setEditable(true);
        chucvu_nv.setEnabled(true);
    }

    public void UnEditable() {
        btn_luu.setEnabled(false);
        ma_nv.setEditable(false);
        ho_nv.setEditable(false);
        ten_nv.setEditable(false);
        gioitinh_nv.setEnabled(false);
        ngaysinh_nv.setEditable(false);
        sdt_nv.setEditable(false);
        luong_nv.setEditable(false);
        chucvu_nv.setEnabled(false);
    }

    private String generateNextMaNV() {
        list = nvBll.getALL();
        String prefix = "nv";
        int maxNumber = 0;
        for (nhanVien item : list) {
            String code = item.getManv();
            if (code == null || code.isBlank()) {
                continue;
            }
            int splitIndex = code.length();
            while (splitIndex > 0 && Character.isDigit(code.charAt(splitIndex - 1))) {
                splitIndex--;
            }
            String numberPart = code.substring(splitIndex);
            if (numberPart.isEmpty()) {
                continue;
            }
            if (splitIndex > 0) {
                prefix = code.substring(0, splitIndex);
            }
            try {
                maxNumber = Math.max(maxNumber, Integer.parseInt(numberPart));
            } catch (NumberFormatException ex) {
                // Ignore codes without numeric suffix.
            }
        }
        return prefix + String.format("%03d", maxNumber + 1);
    }

    private boolean validateInput(String ma, String ho, String ten, String ngaysinh, String sdt, String luongText) {
        if (ma.isEmpty() || ho.isEmpty() || ten.isEmpty() || ngaysinh.isEmpty() || sdt.isEmpty() || luongText.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Vui lòng điền đầy đủ thông tin");
            return false;
        }
        try {
            LocalDate date = LocalDate.parse(ngaysinh, DateTimeFormatter.ofPattern("dd/MM/yyyy"));
            if (date.isAfter(LocalDate.now())) {
                JOptionPane.showMessageDialog(this, "Ngày sinh không được nằm trong tương lai");
                return false;
            }
        } catch (DateTimeParseException ex) {
            JOptionPane.showMessageDialog(this, "Ngày sinh không hợp lệ. Vui lòng nhập theo định dạng DD/MM/YYYY");
            return false;
        }
        if (!sdt.matches("^0\\d{9}$")) {
            JOptionPane.showMessageDialog(this, "Số điện thoại không hợp lệ");
            return false;
        }
        try {
            luong = Long.parseLong(luongText);
            if (luong < 0) {
                JOptionPane.showMessageDialog(this, "Lương không hợp lệ");
                return false;
            }
        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(this, "Lương không hợp lệ");
            return false;
        }
        return true;
    }

    private boolean isDuplicateEmployeeCode(String ma, String oldMa) {
        list = nvBll.getALL();
        for (nhanVien item : list) {
            if (item.getManv().equals(ma) && (oldMa == null || !item.getManv().equals(oldMa))) {
                return true;
            }
        }
        return false;
    }

    private void btn_themActionPerformed() {
        ResetFieldText();
        ma_nv.setText(generateNextMaNV());
        Editable();
        count = 1;
        btn_them.setEnabled(false);
    }

    private void btn_luuActionPerformed() {
        String ma = ma_nv.getText().trim();
        String ho = ho_nv.getText().trim();
        String ten = ten_nv.getText().trim();
        String gioitinh = String.valueOf(gioitinh_nv.getSelectedItem());
        String ngaysinh = ngaysinh_nv.getText().trim();
        String sdt = sdt_nv.getText().trim();
        String chucvu = String.valueOf(chucvu_nv.getSelectedItem());
        String luongText = luong_nv.getText().trim();
        if (!validateInput(ma, ho, ten, ngaysinh, sdt, luongText)) {
            return;
        }

        if (count == 1 && isDuplicateEmployeeCode(ma, null)) {
            JOptionPane.showMessageDialog(this, "Mã nhân viên đã tồn tại");
            return;
        }
        if (count == 2 && isDuplicateEmployeeCode(ma, matmp)) {
            JOptionPane.showMessageDialog(this, "Mã nhân viên đã tồn tại");
            return;
        }

        try {
            nv = new nhanVien(ma, ho, ten, gioitinh, sdt, ngaysinh, chucvu, luong);
            if (count == 1) {
                nvBll.add(nv);
                btn_them.setEnabled(true);
            } else if (count == 2) {
                if (matmp.equals(ma) && hotmp.equals(ho) && tentmp.equals(ten) && gioitinhtmp.equals(gioitinh)
                        && ngaysinhtmp.equals(ngaysinh) && sdttmp.equals(sdt) && chucvutmp.equals(chucvu)
                        && luongTexttmp.equals(luongText)) {
                    JOptionPane.showMessageDialog(this, "Chưa có thông tin nào được sửa đổi");
                    return;
                }
                nvBll.update(nv, matmp);
                btn_sua.setEnabled(true);
            }
            ResetFieldText();
            UnEditable();
            clearTable();
            getTable();
        } catch (Exception ex) {
            System.out.println(ex);
        }
    }

    private void btn_suaActionPerformed() {
        String ma = ma_nv.getText();
        String ho = ho_nv.getText();
        String ten = ten_nv.getText();
        String ngaysinh = ngaysinh_nv.getText();
        String sdt = sdt_nv.getText();
        String luongText = luong_nv.getText();
        if (ma.isEmpty() && ho.isEmpty() && ten.isEmpty() && ngaysinh.isEmpty() && sdt.isEmpty() && luongText.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn đối tượng cần sửa");
            return;
        }
        matmp = ma;
        hotmp = ho;
        tentmp = ten;
        gioitinhtmp = String.valueOf(gioitinh_nv.getSelectedItem());
        ngaysinhtmp = ngaysinh;
        sdttmp = sdt;
        chucvutmp = String.valueOf(chucvu_nv.getSelectedItem());
        luongTexttmp = luongText;
        Editable();
        count = 2;
        btn_sua.setEnabled(false);
    }

    private void btn_xoaActionPerformed() {
        String ma = ma_nv.getText().trim();
        String ho = ho_nv.getText().trim();
        String ten = ten_nv.getText().trim();
        String gioitinh = String.valueOf(gioitinh_nv.getSelectedItem());
        String ngaysinh = ngaysinh_nv.getText().trim();
        String sdt = sdt_nv.getText().trim();
        String chucvu = String.valueOf(chucvu_nv.getSelectedItem());
        String luongText = luong_nv.getText().trim();
        if (ma.isEmpty() && ho.isEmpty() && ten.isEmpty() && ngaysinh.isEmpty() && sdt.isEmpty() && luongText.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn đối tượng cần xóa");
            return;
        }
        int confirm = JOptionPane.showConfirmDialog(this, "Bạn có chắc chắn muốn xóa ?");
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                luong = Long.parseLong(luongText);
                nv = new nhanVien(ma, ho, ten, gioitinh, sdt, ngaysinh, chucvu, luong);
                nvBll.delete(nv);
                ResetFieldText();
                clearTable();
                getTable();
            } catch (Exception ex) {
                System.out.println(ex);
            }
        }
    }

    private void btn_resetActionPerformed() {
        ResetFieldText();
        txtSearch.setText("");
        if (cboMaNVFilter.getItemCount() > 0) {
            cboMaNVFilter.setSelectedIndex(0);
        }
        if (cboGioiTinhFilter.getItemCount() > 0) {
            cboGioiTinhFilter.setSelectedIndex(0);
        }
        if (cboChucVuFilter.getItemCount() > 0) {
            cboChucVuFilter.setSelectedIndex(0);
        }
        UnEditable();
        btn_them.setEnabled(true);
        btn_sua.setEnabled(true);
        clearTable();
        getTable();
    }
}
